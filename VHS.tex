...
\subsection{Construction of VHS (vector bundle version)}
	...
	\begin{frame}[allowframebreaks]
		...
		\begin{Ex}[Canonical Higgs bundle]\label{ex2}
	...
	\begin{frame}[allowframebreaks]{Uniformizing VHS}
		...
		\begin{proposition}\label{uniformizing VHS}
	...
	\begin{frame}
		...
		\begin{corollary}\label{cor9.3}
			Suppose $\g$ is semisimple.If $W\subset \g$ is a sub-Hodge structure such that $[\g^{-1,1},W]\subset W$ then $\dim W^{-1,1}\geq \dim W^{1,-1}$. If equality holds then $W$ is a direct sum of ideals of $\g$.
		\end{corollary}
		
	\end{frame}
	\begin{frame}
		\begin{corollary}\label{cor9.4}
			Suppose $(P,\theta)$ is a uniformizing system of Hodge bundles. Then the following conditions are equivalent:
			\begin{enumerate}
				\item $(P,\theta)$ is stable.
				\item $P\times_K \g$ is a direct sum of stable systems of degree zero.
				\item Each saturated subsystem of Hodge sheaves $W\subset P\times_K \g$ with $\deg(W)\geq 0$ is locally a direct sum of some $\g_i$. 
			\end{enumerate}
		\end{corollary}
		\textit{Proof.}
		$1.\Rightarrow 2.$ Write $P\times_K \g=U_j$, then $U_i$ is stable and since $\mathfrak{u}_j$ is semisimple, it has degree zero. $2.\Rightarrow 3.$ As $P\times_K \g$ is polystable, $\deg W=0$ and $P\times_K \g=W\oplus W'$. In particular, $W'$ is also a system of Hodge bundles. Apply last corollary to $W$ and $W'$, the equality holds for both of them. $3.\Rightarrow 1.$ If $W\subset U_j$ is a saturated subsystem of Hodge sheaves with $\deg W\geq 0$ then $W$ is locally a direct sum of some $\g_i$, hence $W=U_j$ since $U_j$ is finest. \hfill \qed
	\end{frame}
	\begin{frame}
		\begin{theorem}
		Suppose $X$ is compact. $\widetilde{X}\cong \mathbb{D}$ if and only if there is a uniformzing bundle $(P,\theta)$ with structure group $G_0$ corresponding to $\mathbb{D}$ such that $(P,\theta)$ is stable and $\cl_2(P\times_K \g)\cdot [\omega]^{n-2}=0$.
	\end{theorem}
	\noindent \textit{Proof.}
	Each $U_j$ comes from an irreducible Hodge representation of $G'$ to $\mathfrak{u}_j$. Apply Theorem~\ref{prop8.2} we know that $(P,\theta)$ is a uniformizing VHS. Apply Proposition~\ref{uniformizing VHS} we know that $\widetilde{X}\cong \mathbb{D}$. Conversely, apply Proposition~\ref{uniformizing VHS} we get a uniformizing VHS $(P,\theta)$. Hence all Chern classes of $P\times_K \g$ vanish, and it is polystable. Apply above corollary, we know that $(P,\theta)$ is stable.\hfill \qed
	\begin{corollary}\label{cor9.5}
		Let $\mathbb{D}$ be as before, let $X$ be a smooth projective variety. Then for any $\sigma\in \Au(\mathbb{C}/\mathbb{Q})$, if $X$ is uniformized by $\mathbb{D}$, so does $X^{\sigma}$.
	\end{corollary}
	\end{frame}
	\begin{frame}[allowframebreaks]
		\begin{proposition}\label{prop9.6}
			Suppose $X$ is compact and suppose $(P,\theta)$ is a uniformizaing bundle. Then $\theta: T(X)\cong P\times_K \g^{-1,1}\cong \oplus_j V_j$. If the $V_j$ are semistable with degree strictly less than zero, then $(P,\theta)$ is stable. So if $\cl_2(P\times_K \g)\cdot [\omega]^{n-2}=0$, then $(P,\theta)$ is a uniformizing VHS of $X$.	
		\end{proposition}
		\textit{Proof.}
		Suppose $G$ is connected. Then the action of $K$ on $\g_i^{-1,1}$ is irreducible, hence $V_i=P\times_K \g_i^{-1,1}$. The $P\times_K \g_i^{-1,1}$ and their duals $P\times_K \g_i^{1,-1}$ are semistable. The Lie bracket \[ [\cdot,\cdot]: P\times_K \g_i^{-1,1}\otimes P\times_K \g_i^{-1,1}\to P\times_K \g_i^{0,0}\] is surjective, hence the right handside is semistable of degree zero. Suppose $W\subset P\times_K \g_i$ is a subsystem of Hodge sheaves. Then $\deg(W^{0,0})\leq 0$ and $\deg(W^{1,-1}\leq -\rank(W^{1,-1}))\cdot \mu$ and $\deg(W^{-1,1})\leq \rank(W^{1,1})\cdot \mu$ where $\mu(V_i)<0$ is the slope . Therefore
		\[
		\mu(W)\leq (\rank(W^{-1,1})-\rank(W^{1,-1}))\cdot \mu(V_i)\leq 0
		\]
		and if the equality holds, then $W=V_i$ using Corollary~\ref{cor9.3}, hence $V_i$ us stable.

		Suppose $G$ is not connected. Let $G^o$ be the connected component of $G$ and $K^o$ is the connected component of $K$. Then $P/K^o$ is a $K/K^o$-principal bundle which is also a covering space of $X$. And we have the monodromy $\pi_1(X)\to K/K^o$. Find a covering $\pi:X^o\to X$  whose fundamental group is the kernel of the monodromy, then we can reduce the structure group of $P$ from $K$ to $K^o$ over $X^o$. The pullback $\pi^*(V_j)$ are semistable. If we decompose $TX^o=\oplus V_k^o$ with respcet to the $K^o$-action, then $\pi^*(V_j)$ is the direct sum of some $V_k^o$. Hence each $V_k^o$ is semistable with negative degree. By the above discussion for connected $G^o$, they are stable hence $P\times_K \g=P^o\times_{K^o} \g$ is polystable over $X^o$. If $W$ is a subsystem of Hodge sheaves with $\deg(W)\geq 0$ over $X$, then its pullback $\pi^*W$ has zero degree which must be a direct sum of stable subbundle of $P^o\times_{K^o} \g$. Hence by Corollary~\ref{cor9.3}, it is locally direct sum of simple ideals, so does $W$, hence by Corollary~\ref{cor9.4}, $(P,\theta)$ is stable.
	\end{frame}
	\begin{frame}
		\begin{corollary}\label{cor9.7}
			Suppose $X$ is compact. If $T_X=\oplus_{i=1}^{n}L_i$ is a direct sum of lindbundles of negative degree, and if $(\cl_1(X)^2-2\cl_2(X))\cdot [\omega]^{n-2}=0$, then $X$ is uniformized by $\mathbb{B}^n$.
		\end{corollary}
		\textit{Proof.}
		Follows from the discussion in Example~\ref{ex1} that we have a uniformizing bundle $P$ with $K=(\mathbb{C}^*/\{\id,-id\})^n$. And the associated bounded symmetric domain is $\mathbb{B}^n$. Direct calculation shows that $\cl_2(P \times_K \g)=2\cl_2(X)-\cl_1(X)^2$. \hfill \qed
	\end{frame}
	\begin{frame}
		Next call for the canonical Higgs bundle $W$ in Example~\ref{ex2}.
		\begin{proposition}[Miyaoka-Yau inequality]\label{prop9.8}
			Suppose $X$ is compact. If $W$ is a stable then \[
			\left(2\cl_2(X)-\frac{n}{n+1}\cl_1(X)^2\right)\cdot [\omega]^{n-2}\geq 0
			\]
			and if the equality holds, then $X$ is uniformized by $\mathbb{B}^n$.
		\end{proposition}
		If $X$ is a compact curve, then the Chern class condition holds trivially, and $W$ is stable if and only if the genus $g$ of $X$ is larger than $2$. Hence, if $g\geq 2$, $X$ is uniformized by $\mathbb{B}$, this is the classical uniformization.
	\end{frame}
	\begin{frame}[allowframebreaks]
		\begin{proposition}\label{prop9.9}
			Suppose $X$ is a compact K\"ahler surface. If $\cl_1(X)^2\geq 3\cl_2(X)$, $\cl_1(X)^2\geq 0$ and $\cl_1(X)\cdot [\omega]<0$ then $W$ is stable so $\cl_1(X)=3\cl_2(X)$ and $\widetilde{X}=\mathbb{B}^2$.
		\end{proposition}
		\textit{Proof.}
		It suffices to show the dual Higgs bundle $W^*=(\OX,T_X)$ is stable. The only saturated subsheaf of rank two is $T_X$ and it is taken care of by $\cl_1(X)
		\cdot [\omega]<0$. The saturated subsheaves of rank one is invertible subsheaves of $T_X$ who is a line subbundle outside a finite set. Let $L$ be such a subsheaf. We have to show that $\cl_1(L)\cdot [\omega]<\frac{1}{3}\cl_1(X)\cdot [\omega]$. Suppose to the contrary. Let $M=T_X/L$ which is a line bundle in codimension one. Let $N=M^{**}$, then it is a line bundle and $M\hookrightarrow N$ has quotient supported on a finite set. $(\OX,N)$ is a system of Hodge bundles and 
		\[
		\cl_1(N)\cdot [\omega]\leq \frac{2}{3}\cl_1(X)\cdot [\omega]\leq 0
		\]
		so it is stable. The B-G inequality implies $\cl_1(N)^2\leq 0$. $T_X$ is equal to $N+L-(N/M)$ in the Grothendieck group, hence 
		\begin{align*}
			\cl_2(X)&=\cl_1(N)\cl_1(L)-\cl_2(N/M)\\
			&=\cl_1(X)\cl_1(N)-\cl_1(N)^2-\cl_2(N/M)\\
			&\geq \cl_1(N)\cl_1(X)-\cl_1(N)
		\end{align*}
		the last inequality holds because $\cl_2(N/M)$ equals to a nonpositive sum of the point in its support. Therefore 
		\[
		(\cl_1(N)-2\cl_1(X))^2\geq 4(\cl_1(X)^2-3
		\cl_2(X))\geq 0
		\] 
		by our assumption. On the other hand, we apply the Hodge index theorem to $H^{1,1}(X,\mathbb{R})$. The intersection form on it has signature $(1,h^{1,1}-1)$ and we can choose the one positive definition subspace spanned by $\omega$. Hence $\cl_2(X)$ lies in the interior of the negative half of the positive cone, while $3\cl_1(N)$ is outside the cone below $2\cl_1(X)$. Hence $3\cl_1(N)-2\cl_1(X)$ is strictly outside the cone, so its square is strictly negative, hence a contradiction.\hfill \qed
	\end{frame}
\end{document}